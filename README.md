...
The production-style backend, department-aware authorization model, durable document

lifecycle, conversational RAG, streaming responses, observability stack, backup/restore

workflow, security/reliability validation, administrative frontend, and production

operational runbook have been implemented and verified.

The project has passed the final production review checkpoint.
```text
...
9. Frontend                           ✅ Complete
10. Conversational RAG                ✅ Complete
11. Final Production Review           ✅ Complete
```
...
The warning does not represent a failing test. Python application compilation and the

Next.js production build also pass.
...
| Production Backup | systemd, PostgreSQL dump, document storage, Qdrant snapshot |

| Remote Backup | Amazon S3 with versioning and lifecycle retention |

| Backup Credentials | AWS IAM / Roles Anywhere |

| Production Scheduling | systemd timer |
...
The query workflow supports durable conversations and token-by-token streaming responses.

New conversations are created on the first question, and a failed first-turn stream cleans

up its just-created conversation.

Administrative user, role, department, and document workflows are also available through

the frontend.

The Next.js production build has been verified successfully.

---

## Conversational RAG

Conversation persistence is implemented as durable PostgreSQL application state.

The conversation workflow supports:
```text
Create conversation
        ↓
...
The query API supports an optional `conversation_id`. Conversation ownership is checked

before retrieval or generation, so one user cannot access another user's conversation.

Conversational generation uses a bounded history of recent user messages as conversational

context. Conversation history is not treated as authoritative evidence; retrieved,

authorized document chunks remain the grounding source.
...
User and assistant messages are persisted only after a streaming response completes

successfully. Provider failures and interrupted streams do not leave a successful assistant

message behind.

Authorization remains outside the LLM: conversation context does not bypass the normal

department authorization and retrieval validation pipeline.
...
The warning comes from the Starlette test-client / AnyIO deprecation path and does not
...
The current production baseline has been exercised with:
```text
...
systemd 10-hour backup timeout              ✅
Local SHA-256 verification                  ✅
S3 archival and remote object verification  ✅
Remote retention                            ✅
PostgreSQL restore drill                    ✅
Document restore drill                      ✅
Qdrant restore drill                        ✅
Python compilation                          ✅
Backend test suite                          ✅ 320 passed
Frontend production build                   ✅
Final Git working tree                      ✅ clean
```

The production backup pipeline creates durable local backups, verifies the checksum

manifest, archives verified backups to Amazon S3, verifies the remote object set, and

applies remote retention.

Host-installed systemd helpers are:
```text
...
The application document storage remains the Docker volume:
```text
secure-rag_app_storage
```

mounted inside application containers at:
```text
/app/storage
```

The obsolete Compose `storage-init` container is not part of the current production
